...
public:
    ...
    // Estimate of performance: O(n)
    // Short rationale for estimate: Iterate through every WayIDs
    std::vector<WayID> all_ways();

    // Estimate of performance: O(n)
    // Short rationale for estimate: Iterate through all coords to calculate distance
    // costs linear. Add new elements to unordered_map, then to unordered_multimap cost O(1)
    bool add_way(WayID id, std::vector<Coord> coords);

    // Estimate of performance: O(1) then O(n)
    // Short rationale for estimate: Find element in map, then add to a vector
    std::vector<std::pair<WayID, Coord>> ways_from(Coord xy);

    // Estimate of performance: O(1)
    // Short rationale for estimate: Search key in map
    std::vector<Coord> get_way_coords(WayID id);

    // Estimate of performance: O(n)
    // Short rationale for estimate: Clear maps
    void clear_ways();

    // Estimate of performance: O(V+E), V: no. of vertices, E: no. of edges
    ...
    // Short rationale for estimate: Find and erase elements in unordered_map(s)
    ...
    // Estimate of performance:O(V+ElogV)
    // Short rationale for estimate: Dijkstra algorithm
    std::vector<std::tuple<Coord, WayID, Distance>> route_shortest_distance(Coord fromxy, Coord toxy);

    // Estimate of performance: O(n)
    // Short rationale for estimate: Iterate through unordered_map 3 times
    Distance trim_ways();
...
private:
    ...
    struct Place{
        ...
        Place (const PlaceID &id_, const Name &name_, const PlaceType &type_,
               const Coord &coord_, const Id_TypeCoord_ptr &placeid_typecoord_ptr_){
            placeid = id_;
            placename = name_;
            placetype = type_;
            placecoord = coord_;
    ...
    // Phase 2:
    using Color = int;
    const Color WHITE = 0, GREY = 1, BLACK = -1;
    std::shared_ptr<Color> white_ptr = std::make_shared<Color>(WHITE);
    std::shared_ptr<Color> grey_ptr = std::make_shared<Color>(GREY);
    std::shared_ptr<Color> black_ptr = std::make_shared<Color>(BLACK);
    struct Point {
        Coord point_coord;
        using Point_ptr = std::shared_ptr<Point>;
        std::unordered_multimap<Point_ptr, std::pair<WayID, Distance>> crossroad;
        Distance d;
        std::shared_ptr<Color> color;
        Point_ptr prev_point;
        Point (const Coord &point_coord_){
            point_coord = point_coord_;
        }
    };
    using Point_ptr = std::shared_ptr<Point>;
    using Way = std::vector<Coord>;
    std::unordered_map<WayID, Way> way_data;
    std::unordered_map<Coord, Point_ptr, CoordHash> all_crossroads;

    // Reset color of visited coords
    void reset_way_color();

    // Calculate the distance of a way with given coords
    Distance way_distance(Way coords);

    // Calculate total distance of every ways. Only use in trim_ways()
    Distance total_way_distance();
};
